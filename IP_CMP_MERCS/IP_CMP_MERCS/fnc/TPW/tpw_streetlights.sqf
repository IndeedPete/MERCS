...
//if !(isnil "tpw_streetlights_active") exitwith {hint "TPW STREETLIGHTS already running."};
if (count _this < 3) exitwith {hint "TPW STREETLIGHTS incorrect/no config, exiting."};
WaitUntil {!isNull FindDisplay 46};

// VARIABLES
private ["_colsel","_moths","_lightarray","_lastpos","_colour","_incan","_sodium","_fluoro"];

tpw_streetlights_version = "1.01"; // Version string
...
_fnc_light =
	...
	if !(_lamp in _lightarray) then 
		...
		if (_moths == 1) then 
		{
			_ball = "sign_sphere25cm_F" createvehicle position _lamp;
