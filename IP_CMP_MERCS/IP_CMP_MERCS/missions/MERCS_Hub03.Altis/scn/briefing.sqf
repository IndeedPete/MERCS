...
switch (_mission) do {
	case "MMain06" : {
		...
		if ("B_Heli_Light_01_F" in (player getVariable ["IP_CampVehicles", []])) then {[player, [("t" + _mission + "InsertionHeli"), ("t" + _mission)], ["Go to the heli pad to insert by helicopter!", "Insert By Helicopter", "Helicopter"], IP_Heli, false, 1, true, "heli"] call BIS_fnc_taskCreate};
	...
	default {
		...
		if ((count IP_GarageVehicles) > 0) then {[player, [("t" + _mission + "InsertionCar"), ("t" + _mission)], ["Go to the garage to insert by car!", "Insert By Car", "Garage"], IP_Garage, false, 1, true, "car"] call BIS_fnc_taskCreate};
		if ("B_Heli_Light_01_F" in (player getVariable ["IP_CampVehicles", []])) then {[player, [("t" + _mission + "InsertionHeli"), ("t" + _mission)], ["Go to the heli pad to insert by helicopter!", "Insert By Helicopter", "Helicopter"], IP_Heli, false, 1, true, "heli"] call BIS_fnc_taskCreate};
		if ("B_G_Boat_Transport_01_F" in (player getVariable ["IP_CampVehicles", []])) then {[player, [("t" + _mission + "InsertionBoat"), ("t" + _mission)], ["Go to the beach to insert by boat!", "Insert By Boat", "Boat"], IP_Boat, false, 1, true, "boat"] call BIS_fnc_taskCreate};
	};
};