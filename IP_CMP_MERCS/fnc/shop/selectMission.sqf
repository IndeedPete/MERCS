...
_currentTask = player getVariable ["IP_Mission", ""];
...
player setVariable ["IP_Mission", _task];

if ((getText(missionConfigFile >> "stage")) == "A") then {
	[player, ("t" + _task), [_desc, _title, ""], IP_Commander, true, 1] call BIS_fnc_taskCreate;
} else {
	IP_BriefingDone = nil;
	[player, ("t" + _task), [_desc, _title, ""], nil, true, 1] call BIS_fnc_taskCreate;
	[player, [("t" + _task + "Briefing"), ("t" + _task)], ["Report in for briefing!", "Briefing", "Briefing"], IP_Commander, true, 1] call BIS_fnc_taskCreate;
};