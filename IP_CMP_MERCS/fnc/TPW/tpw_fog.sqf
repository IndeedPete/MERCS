...
2 - Call it with 0 = [250,10,1,1,1,1,1] execvm "\scripts\tpw_fog.sqf"; where 250 = radius around player to give units foggy breath (m). 10 = delay until functions start (sec), 1 = foggy breath, 1 = ground fog, 1 = rain fog, 1 = heat haze , 1 = snow
...
//if !(isnil "tpw_fog_active") exitWith {hint "TPW FOG already running."};
WaitUntil {!isNull FindDisplay 46};

// VARIABLES
tpw_fog_version = "1.18"; // version string
...
//HIDE BALL
tpw_fog_fnc_hideball = 
	{
	private ["_ball"];
	_ball = _this select 0;
	_ball setObjectTextureGlobal [0,"#(argb,8,8,3)color(0.0,0.0,0.0,0.0,ca)"];
	};		
...
sleep 1;
[tpw_fog_fnc_breathe, 0.5] call cba_fnc_addPerFrameHandler;
